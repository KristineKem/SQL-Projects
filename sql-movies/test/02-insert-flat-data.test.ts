...
const insertMovies = (movies: Movie[]) => {
  return(
    `insert into movies (imdb_id, original_title, homepage, tagline, overview, release_date, popularity, budget, budget_adjusted, 
      revenue, revenue_adjusted, runtime) values ` +
    movies.map(movie => (`('${escape(movie.imdbId)}', '${escape(movie.originalTitle)}', '${escape(movie.homepage)}', 
      '${escape(movie.tagline as string)}', '${escape(movie.overview)}', '${escape(movie.releaseDate)}', ${movie.popularity}, 
        ${movie.budget}, ${movie.budgetAdjusted}, ${movie.revenue}, ${movie.revenueAdjusted}, ${movie.runtime})`)).join(", ")
  );
};
