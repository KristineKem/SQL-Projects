...
describe("Simple Queries", () => {
    ...
    it("should select top 3 developers with most apps published", async done => {
        const query = `select count(title) as count, developer from apps
        group by developer order by count desc, developer limit 3`;
    ...
    it("should select count of reviews created in year 2014, 2015 and 2016", async done => {
        ...
        where year in ('2014', '2015', '2016') group by year`;
        const result = await db.selectMultipleRows(query);
        expect(result).toEqual([
